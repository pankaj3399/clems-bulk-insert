...
async function deleteOldDocuments(client, urlDate) {
  ...
  const tenDaysBeforeUrlDate = moment(urlDate)
    .subtract(10, "days")
    .format("YYYY-MM-DD");
  const result = await collection.deleteMany({
    date: { $lt: tenDaysBeforeUrlDate },
  });

  console.log(
    `Deleted ${result.deletedCount} documents older than 10 days from ${urlDate}`,
  );
}
...
async function generateData() {
  ...
  return new Promise((resolve, reject) => {
    axios
      .get(CSV_URL, { responseType: "stream" })
      .then((response) => {
        response.data
          .pipe(csvParser())
          .pipe(
...
async function performInsertion() {
  ...
  try {
    ...
    const hasDocumentsForDate = await checkForDocumentsByDate(
      client,
      dateFromUrl,
    );
    if (hasDocumentsForDate) {
      console.log(
        `Documents for date ${dateFromUrl} already exist. Exiting...`,
      );
      return;
    }
